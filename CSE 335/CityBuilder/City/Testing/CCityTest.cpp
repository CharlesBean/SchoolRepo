...
namespace Testing
{
	...
	public:
		...
		virtual void VisitLandscape(CTileLandscape *landscpae) { mNumLandscapes++; }
	...
	public:
		...
		TEST_METHOD(TestCCityZoningIterator)
		{
			...
			*/
			...
			Assert::IsTrue(tile1 == *iter1, L"First item correct");
			++iter1;
			Assert::IsTrue(tile3 == *iter1, L"Second item correct");
			++iter1;
			Assert::IsFalse(iter1 != iter2);
		...
		TEST_METHOD(TestCCityVisitor)
		{
			...
			// Have the city accept the visitor
			CTestVisitor visitor;
			city.Accept(&visitor);

			// Make several assertions to see if the visitor correctly counted each type
