...
void CAnimChannel::DeleteKeyframe(int keyframe)
{
	// If to right
	if (mKeyframe1 == (mKeyframes.size() -1) && mKeyframe2 == -1) 
	{
		// Delete the keyframe from the vector
		mKeyframes.erase(mKeyframes.begin() + mKeyframe1);

		// Decrement keyframe 1
		mKeyframe1--;
	}
	else if (mKeyframe1 >= 0 && mKeyframe2 >= 0) // If in between
	{
		// Delete the keyframe from the vector
		mKeyframes.erase(mKeyframes.begin() + mKeyframe1);

		// Decrement keyframe 1 & 2
		mKeyframe1--;
		mKeyframe2--;
	}
